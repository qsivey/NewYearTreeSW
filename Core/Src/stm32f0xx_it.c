...
void EXTI0_1_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI0_1_IRQn 0 */
	...
	static ui32 paramsSaveTime = 0;

	if (__HAL_GPIO_EXTI_GET_IT(BUT1_Pin))
	{
	...
	if (__HAL_GPIO_EXTI_GET_IT(BUT2_Pin))
	{
		if (HAL_GPIO_ReadPin(BUT2_GPIO_Port, BUT2_Pin) == GPIO_PIN_RESET)
			paramsSaveTime = HAL_GetTick();

		else if (HAL_GPIO_ReadPin(BUT2_GPIO_Port, BUT2_Pin) == GPIO_PIN_SET)
		{
			if ((HAL_GetTick() - paramsSaveTime) > 1500)
				nytSave()
...
void EXTI2_3_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI2_3_IRQn 0 */
	...
	if (__HAL_GPIO_EXTI_GET_IT(BUT3_Pin))
	{
		NewYearTree.speed++;
