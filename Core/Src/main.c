...
//  MX_TIM6_Init();
  ...
  /* USER CODE BEGIN 2 */
	...
	/* Read params */
	*((ui32*)(&NewYearTreeSettings)) = InternalFlashReadWord(FLASH_SETTINGS_ADDRESS);
	*((ui32*)((&NewYearTreeSettings) + 4)) = InternalFlashReadWord(FLASH_SETTINGS_ADDRESS + 4);

	if (NewYearTreeSettings.mode == NYTM_EMPTY_FLASH)
	{
	...
	{
		NewYearTree.saturation = NewYearTreeSettings.saturation;
		NewYearTree.brightness = NewYearTreeSettings.brightness;
		NewYearTree.speed = NewYearTreeSettings.speed;

		NewYearTree.mode = NewYearTreeSettings.mode;
	}
...
//			case NYTM_MANUAL_RGB_12 :		NYT_LED_Mode12();			break;
			...
			case NYTM_MANUAL_SC_7 :
				if (NewYearTree.staticColorState != NYTSCS_WARM_LIGHT)
				{
					NewYearTree.staticColorState = NYTSCS_WARM_LIGHT;
					NYT_LED_ModeSimpleColor();
				}
